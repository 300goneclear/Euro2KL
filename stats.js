...
// ---- HOTFIX JS (removes scoreboard, fixes team names, robust fallbacks) ----
...
function extractPlayersWithTeam(game){
  const arrays=findPlayerArrays(game); const out=[];
...
async function loadGamePreferBox(){
  try{ const box=await tryFetchJSON(DATA_BASES,BOX_FILE); return {source:'box', game: box.game||box}; }catch(e){ /* continue */ }
  const raw=await tryFetchJSON(DATA_BASES,GAMES_FILE); const games=Array.isArray(raw)?raw:(raw.games||raw.schedule||raw.data||[]);
  if(!games.length) throw new Error('No games found.'); const sorted=[...games].sort((a,b)=>parseGameTime(b)-parseGameTime(a));
  const game=sorted.find(g=>/final|complete|completed|ended/i.test(String(g.status||'')))||sorted[0]; return {source:'games', game};
}
...
async function buildTeamLeaders(){
  ...
  try{
    ...
    try{
      ...
      if(Array.isArray(tstats)&&tstats.length){ const cats=[['POINTS PER GAME','ppg',1],['REBOUNDS PER GAME','rpg',1],['ASSISTS PER GAME','apg',1],['STEALS PER GAME','spg',1],['BLOCKS PER GAME','bpg',1]];
    ...
    try{
      const box=await tryFetchJSON(DATA_BASES,BOX_FILE); const game=box.game||box;
      ...
      function compute(team,label){
        ...
        const pl=Array.isArray(team.players)?team.players:[]; const sum=(obj,k,alts)=> Number(obj[k] ?? (alts?alts(obj):0) || 0);
...
window.addEventListener('DOMContentLoaded', ()=>{
  try{ if(document.querySelector('#player-leaders')) buildPlayerLeaders(); }catch(e){ console.error(e); }
  try{ if(document.querySelector('#team-leaders')) buildTeamLeaders(); }catch(e){ console.error(e); }
});
